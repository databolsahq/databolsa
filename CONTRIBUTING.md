...
Dentro do workspace, rode a CLI pelo script raiz — `npx @databolsa/cli` resolve o
pacote local sem o binário linkado e falha com `databolsa: not found`:

```bash
bun run cli getHealth --json
```

Fora do workspace, `npx -y @databolsa/cli` usa o pacote publicado e funciona
normalmente.
