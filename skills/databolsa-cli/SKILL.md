...
2. Discover the current command rather than inventing a path, option, filter, or unit:
   ...
   When the exact request body, response schema, or enum matters, query only the
   relevant part of the live contract. See [the OpenAPI reference](references/openapi.md).
