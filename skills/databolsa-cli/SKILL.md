...
Brazilian equities · FIIs · funds, ETFs, BDRs and indexes · fixed income and macro
...
add up, a rating's `rating_notch` compares only within the same `scale`, and
`listOtcQuotes` withholds size in seven families by disclosure rule rather than by
gap. Read [references/domains.md](references/domains.md) for the surface you are
