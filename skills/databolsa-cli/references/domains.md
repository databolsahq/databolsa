...
- **The object graph:** one object per real-world thing, with every code that
  points at it, and typed relations between them. Start here when a question
  links two things ("who holds X", "how is X connected to Y"): `resolveObject`
  turns a code or a name into candidates, and `getObject` returns two maps — the
  relations you can traverse and the data chapters the object has, each with the
  operation and parameter already resolved.

  Two identity rules decide whether an answer is right, and both are recent:

  - **A share is not its issuer.** `resolveObject PETR4` returns the SHARE
    (`kind: equity_security`), not Petrobras. Price, indicators and dividends
    belong to the share and differ between PETR3 and PETR4. To reach the issuer,
    traverse `issued` inbound; the company still lists its `tickers` and
    `share_classes`. A listed fund's quota is NOT a separate object — a fund has
    one quota, so HGLG11 resolves to the fund itself.
  - **An indicator is not the series that publishes it.** `ipca` is the concept
    (`kind: indicator`); `bcb_sgs:433` is the BCB's monthly publication of it
    (`kind: data_series`), and they are linked by `measures`. Debentures index to
    the CONCEPT, so "which papers pay IPCA" does not depend on which series
    someone cited. The concept carries no figures of its own — traverse
    `measures` to reach a series and ask for its history there.

  Counting has two answers and they mean different things: `relationship_count`
  counts the DISTINCT objects on the other side and is what "how many" means;
  `assertion_count` counts the statements backing them, one per source and
  contiguous period, and is always greater or equal. The gap is source
  redundancy, never size. `magnitude` is the SIZE of a relation (BRL, portfolio
  share, index weight) and never documentary proof — proof is `getObjectEvidence`,
  which returns the passage with its protocol and page.
