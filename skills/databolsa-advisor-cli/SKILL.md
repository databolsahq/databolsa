...
Use `databolsa-advisor` when installed, or `npx --yes
@databolsa/advisor-cli` for a one-off invocation outside a DataBolsa source
checkout. Inside the source checkout, npm can shadow the published binary with
the local workspace package; use `bun run cli:advisor --` there.

```bash
databolsa-advisor --list
npx --yes @databolsa/advisor-cli advisorGetMe --help
bun run cli:advisor -- advisorGetMe --json # source checkout
```
