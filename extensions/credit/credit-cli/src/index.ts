...
await runCli({
  ...
  branding: {
    ...
    envWorkspace: "DATABOLSA_CREDIT_WORKSPACE",
  ...
  apiErrorHint: (status) => {
    if (status === 401) {
      return "Defina DATABOLSA_CREDIT_API_KEY com sua chave pessoal db_live_... (crie em databolsa.com/conta) e DATABOLSA_CREDIT_WORKSPACE com o id da organização.\n";
    }
    if (status === 402) {
