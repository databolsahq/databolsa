 ...
 *   DATABOLSA_ADVISOR_WORKSPACE id da organização em que agir (header x-databolsa-workspace)
 */
...
await runCli({
  ...
  branding: {
    ...
    envWorkspace: "DATABOLSA_ADVISOR_WORKSPACE",
  ...
  apiErrorHint: (status) => {
    if (status === 401) {
      return "Defina DATABOLSA_ADVISOR_API_KEY com sua chave pessoal db_live_... (crie em databolsa.com/conta) e DATABOLSA_ADVISOR_WORKSPACE com o id da organização.\n";
    }
    if (status === 402) {
