 ...
 * (`{ baseUrl, fetch }`).
 */
export function wallet(opts: { workspace?: string } = {}) {
  return (ctx: { baseUrl: string; fetch: FetchLike }): DataBolsaWallet =>
    new DataBolsaWallet({ baseUrl: ctx.baseUrl, fetch: ctx.fetch, workspace: opts.workspace });
}
