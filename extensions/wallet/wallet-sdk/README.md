...
- Use `npx -y @databolsa/wallet-cli --list` para descoberta interativa fora do
  checkout; dentro dele, use `bun run cli:wallet -- --list`. A CLI principal
  também mantém `databolsa wallet` como atalho de conveniência.
- A credencial ou conexão OAuth já determina um único workspace. Para usar outro,
  emita outra credencial ou reconecte; não altere o alvo por opção do cliente.
