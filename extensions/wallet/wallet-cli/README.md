...
Sem instalação, fora de um checkout do DataBolsa:
...
```bash
npm install -g @databolsa/wallet-cli
databolsa-wallet listPortfolios --json
```

Dentro do checkout, o npm pode selecionar o workspace local sem disponibilizar
o binário ainda não compilado. Use o script da raiz:

```bash
bun run cli:wallet -- --list
bun run cli:wallet -- listPortfolios --json
```
...
| `DATABOLSA_API_KEY` | Credencial vinculada a um único workspace na emissão. |

A CLI usa somente o workspace gravado na credencial ou na conexão OAuth. Para
agir em outro workspace, emita outra credencial ou refaça a conexão; não reutilize
a mesma chave com um seletor de workspace.
