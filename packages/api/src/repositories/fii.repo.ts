...
export const fiiRepo = {
  ...
  async list(opts: FiiListOpts) {
    ...
    // NULLS LAST: no Postgres, DESC traz NULL primeiro — sem isso, FIIs sem o
    // indicador escolhido (ex.: dy_12m nulo p/ fundo sem distribuição) subiam ao
    // topo de `sort=-dy_12m`. Empurra os nulos para o fim em ambas as direções.
    const primary = sql`${col} ${opts.sortDir === "asc" ? sql`asc` : sql`desc`} nulls last`;
    return db
      .select({
        ticker: fiiProfile.ticker,
