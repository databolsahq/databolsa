...
export const indicatorRepo = {
  ...
  async screen(opts: ScreenOpts) {
    ...
    // NULLS LAST: no Postgres, DESC traz NULL primeiro — empurra ativos sem o
    // indicador escolhido (ex.: dy12m nulo) para o fim em vez do topo.
    const order = sql`${col} ${opts.sortDir === "asc" ? sql`asc` : sql`desc`} nulls last`;
    return db
      .select({
        ticker: pi.ticker,
        name: pi.companyName,
