...
const tesouroQuery = paginationQuery.extend({
  type: z
    .enum(["LTN", "NTN-F", "NTN-B", "NTN-B-Principal", "LFT", "RENDA+", "EDUCA+"])
    .optional()
    .describe("Tipo do título: LTN (prefixado), NTN-F (prefixado c/ juros), NTN-B(-Principal) (IPCA+), LFT (Selic), RENDA+, EDUCA+."),
  maturity: isoDate.optional().describe("Filtra por vencimento (YYYY-MM-DD)."),
  date: isoDate.optional().describe("Data de referência das taxas/preços (default: mais recente)."),
});

const curveQuery = z.object({
  date: isoDate.optional().describe("Data de referência da curva (default: mais recente)."),
  kind: z
    .enum(["nominal", "real"])
    .default("nominal")
    .describe("nominal = juros prefixados (LTN/NTN-F); real = juros reais IPCA+ (NTN-B)."),
});
