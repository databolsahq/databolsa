...
const seriesParam = z.object({
  source: z
    .string()
    .min(1)
    .describe("Fonte: bcb_sgs | bcb_focus | ibge_sidra | fred | ipeadata | tesouro_direto."),
  series_id: z
    .string()
    .min(1)
    .describe(
      "ID da série na fonte. Em bcb_sgs: 432=Selic meta (%a.a.), 12=CDI diário, 433=IPCA mês, " +
        "13522=IPCA 12m, 1=USD/BRL, 189=IGP-M. Use listSeries para descobrir outras.",
    ),
});
...
const seriesQuery = z
  .object({
    ...
    accumulated: z
      .enum(["none", "12m"])
      .default("none")
      .describe("Transformação: none (default) ou 12m (acumulado 12 meses, p/ séries de variação como IPCA)."),
  })
...
export const series = new Hono()
  ...
  .get(
    ...
    describeRoute({
      ...
      description:
        "Série temporal (data, valor) de uma fonte macro (BCB SGS, IBGE SIDRA, FRED…). `source`+`series_id` " +
        "identificam a série; `accumulated=12m` acumula variações. Ex.: bcb_sgs/432 = Selic meta.",
