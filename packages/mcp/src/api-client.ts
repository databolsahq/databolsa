 ...
 *
 * Espelha o padrão de `apps/web/lib/api/http-client.ts`: Bearer opcional,
 * parsing de `application/problem+json` e degradação graciosa de 501/404
 * ("fora do preview"). A API não mantém dados de usuário; a chave é um cheque
 * opcional no modo self-host. Base URL e chave vêm do ambiente.
 */
...
export class ApiClient {
  ...
  async fetchOpenApi(): Promise<unknown> {
    const res = await fetch(`${this.origin}/openapi.json`, { headers: { accept: "application/json" } });
