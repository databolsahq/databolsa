...
Servidor [MCP](https://modelcontextprotocol.io) para a API pública e os recursos
autenticados da conta no DataBolsa.
...
geradas no startup a partir do contrato OpenAPI vivo e declaram schemas de
entrada e saída. Respostas bem-sucedidas incluem `structuredContent` validado,
além do JSON em texto para compatibilidade com clientes MCP antigos.
...
adaptador fino do contrato da API.
...
| `DATABOLSA_MCP_PROFILE` | vazio | Perfil da sessão (`context`, `market`, `credit`, `account`, `full`). Expõe só as operações das capacidades do perfil, declaradas no próprio contrato (`x-profiles`). Vazio = superfície completa. |

### Perfis

O contrato OpenAPI declara perfis nomeados (`x-profiles`) e carimba cada operação com uma
capacidade (`x-capability`). Com `DATABOLSA_MCP_PROFILE=context`, por exemplo, a sessão expõe
apenas o grafo de objetos, documentos, eventos e saúde do serviço (~30 tools em vez de 172) —
útil para agentes, que gastam menos contexto com definições de tool. Perfil desconhecido é
erro no startup, com a lista dos válidos; a autorização real continua na API, por chave e
escopo — o perfil reduz exposição e contexto, não é fronteira de segurança.
...
As consultas de mercado são somente leitura. As operações de carteira e teses
alteram dados da conta e devem ser confirmadas pelo usuário antes da chamada. O
DataBolsa não executa ordens nem toma decisões de investimento.
