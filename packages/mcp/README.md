...
### Alternar entre hospedada e local

Use `--scope` para escolher onde a configuração vive: `local` (privada, só sua
máquina, ideal para a chave) sobrepõe a `project` (committada no `.mcp.json`). Para
desenvolver contra um core local, aponte a URL para ele e dispense a chave:

```bash
# self-hosted / dev local (core aberto em :8081)
...
| `DATABOLSA_API_KEY` | vazio | Token Bearer. **Obrigatório na API hospedada** (`api.databolsa.com`); opcional num core self-hosted aberto. |
...
Um core self-hosted aberto dispensa a chave (veja [API Local](#api-local)).
