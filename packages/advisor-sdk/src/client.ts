 ...
 * Autentica com chave de organização (`db_org_...`) ou chave pessoal de membro
 * (`db_live_...`); no portal, cookies de sessão via `credentials: "include"`.
 */
...
export interface AdvisorClientOptions {
  ...
  baseUrl?: string;
  /** Chave `db_org_...` ou `db_live_...`. Omitida no portal (sessão via cookie). */
  apiKey?: string;
...
export class DataBolsaAdvisor {
  ...
  listOrgKeys(org: string) {
    return this.request<Ok<"/v1/advisor/orgs/{org}/keys", "get">>("GET", `/v1/advisor/orgs/${enc(org)}/keys`);
  }

  createOrgKey(org: string, body: Body<"/v1/advisor/orgs/{org}/keys", "post">) {
    return this.request<Ok<"/v1/advisor/orgs/{org}/keys", "post">>("POST", `/v1/advisor/orgs/${enc(org)}/keys`, body);
  }

  revokeOrgKey(org: string, keyId: string) {
    return this.request<Ok<"/v1/advisor/orgs/{org}/keys/{keyId}", "delete">>(
      "DELETE",
      `/v1/advisor/orgs/${enc(org)}/keys/${enc(keyId)}`,
    );
  }
