...
-- DY 12m = Σ distribuições dos ÚLTIMOS 12 MESES ÷ preço. Antes somava as últimas 12
-- LINHAS (rn<=12): p/ pagadores irregulares (VSEC, TSNC) varria ANOS e dividia por um
-- preço atual descolado → yield absurdo (até 13478%). Agora: janela de 12 meses a
-- PARTIR do último pagamento de CADA fundo, com teto de 3% da cota por mês — um
-- rendimento mensal de FII gira em ~1%; >3% é shares_issued corrompido no informe
-- (XPML 233/cota) ou preço fóssil de cota ilíquida (VSEC a 0,21). O teto também zera
-- fundos que pararam de pagar há muito (distribuições antigas > 3% da cota atual).
-- LIMITAÇÃO: `value_per_share` vem de Rendimentos_Distribuir (saldo a distribuir do
-- informe), proxy imperfeito do pago de fato — fundos que distribuem 100%/mês zeram o
-- saldo e somem do DY. Fonte correta = anúncios B3/FNET (backlog).
dist_12m as (
...
select
    ...
    -- Rendimentos_Distribuir 12m (resultado distribuível). Difere do DY quando o
    -- fundo retém ou distribui reservas. Ver docs/data-quality/...verification.md.
    case when p.preco > 0 and ffo.shares_max > 0 and ffo.ffo_12m > 0
        then round(ffo.ffo_12m / (p.preco * ffo.shares_max) * 100, 4) end as ffo_yield,
    tr.vacancia_fisica,
    tr.cap_rate,
