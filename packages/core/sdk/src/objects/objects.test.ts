...
describe("ligação derivada do contrato", () => {
  ...
  test("caminho vem do path do contrato: `getSeries` recebe source e series_id posicionais, nessa ordem", async () => {
    const { db, chamadas } = clienteFalso({ getSeries: (async (...a: unknown[]) => (chamadas.push({ op: "getSeries", args: a }), {})) as never, getObject: (async () => ({ id: "pub_s", kind: "data_series", subkind: null, name: "IPCA", keys: [], links: [], declared_facts: [], aspects: [{ name: "series", description: "", grain: "object", shape: "series", operation: "getSeries", parameter: "series_id", value: "433", defaults: { source: "bcb_sgs", series_id: "433" } }] })) as never });
    const s = await new Objects(db).get("pub_s");
    await s.aspect("series", { from: "2025-01-01" });
    expect(chamadas.at(-1)).toEqual({ op: "getSeries", args: ["bcb_sgs", "433", { from: "2025-01-01" }] });
  });
