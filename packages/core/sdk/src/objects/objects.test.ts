...
describe("resolveOne", () => {
  ...
  test("sem `kind`, `instanceof` estreita a união; `kind` fica aberto a tipo que o contrato publique depois", async () => {
    ...
    if (h instanceof InstrumentHandle) {
      ...
      expect(typeof h.issuer).toBe("function");
    } else {
    ...
    const escolhido = ((x: AnyObjectHandle) => {
      ...
      switch (x.kind) {
        case "company":
          return x.instruments; // CompanyHandle
        case "equity_security":
          return x.market.corporate_events.list; // EquitySecurityHandle
        default:
          return null;
      }
    })(h);
...
describe("jornada 1 — equity_security → market.corporate_events.list", () => {
  ...
  test("`at` mora no handle: fatos recebem `at`, retrato recebe `at`, série recebe `to`", async () => {
    ...
    await petr4.market.corporate_events.list();
    ...
    expect(porOp.listCorporateEvents?.[1]).toEqual({ to: "2025-06-30" });
  ...
  test("`at` num capítulo que só responde o vigente é recusado, não devolvido como se fosse daquela data", async () => {
    const { db } = clienteFalso({
      getObject: (async () => ({
        id: "pub_us", kind: "company", subkind: null, name: "APPLE", keys: [], links: [], declared_facts: [],
        aspects: [{ name: "us_filings", function: "us.filings.list", description: "", grain: "object", shape: "event", operation: "listUsFilings", parameter: "ticker", value: "AAPL", defaults: { ticker: "AAPL" }, available: true }],
      })) as never,
    });
    const cia = (await new Objects(db).get("pub_us")).at("2024-12-31");
    const erro = await cia.aspect("us_filings").catch((e: unknown) => e);
    expect(erro).toBeInstanceOf(TemporalCutUnsupportedError);
    expect((erro as TemporalCutUnsupportedError).operation).toBe("listUsFilings");
  });
...
describe("jornada 2 — company → issued → instrument", () => {
  ...
  test("capítulo com available:false é chamado mesmo assim e responde vazio — não é erro", async () => {
    const { objects, chamadas } = montar();
    const petrobras = await objects.resolveOne("33000167000101", { kind: "company" });
    expect((await petrobras.aspects()).find((a) => a.name === "documents")?.available).toBe(false);
    const r = (await petrobras.aspect("documents")) as { data: unknown[] };
    expect(r.data).toEqual([]);
    expect(chamadas.at(-1)?.op).toBe("listCompanyDocuments");
  });

  // O INSTRUMENTO deixou de abrir capítulo de rota em 04/09/2026: a nota da agência é a aresta
  // `rates` e o secundário é série de fatos. O que resta na jornada é a travessia.
  test("o instrumento resolvido não publica capítulo de rota nenhum", async () => {
    const { objects } = montar();
    const deb = await objects.resolveOne("BRPETRDBS043", { kind: "instrument" });
    expect((await deb.aspects()).map((a) => a.name)).toEqual(["facts", "properties"]);
  });
...
describe("companhia versus papel", () => {
  test("capítulo de papel numa companhia com dois papéis exige escolher", async () => {
    const { objects } = montar();
    const petrobras = await objects.resolveOne("33000167000101", { kind: "company" });
    // o tipo já exige `series` em `market.corporate_events.list`; pela porta genérica, o runtime recusa
    const erro = await petrobras.fn("market.corporate_events.list").catch((e: unknown) => e);
    expect(erro).toBeInstanceOf(AmbiguousPaperError);
    expect((erro as AmbiguousPaperError).tickers).toEqual(["PETR3", "PETR4"]);
  });
  ...
  test("`series` aceita o ticker ou a classe; `paper()` devolve o handle do papel pela aresta", async () => {
    const { objects, chamadas } = montar();
    const petrobras = await objects.resolveOne("33000167000101", { kind: "company" });
    await petrobras.market.corporate_events.list({ series: "PN", from: "2025-01-01" });
    expect(chamadas.at(-1)).toEqual({ op: "listCorporateEvents", args: ["PETR4", { from: "2025-01-01" }] });
    await petrobras.market.corporate_events.list({ series: "petr3" });
  ...
  test("capítulo que o objeto não publica é erro com a lista do que ele publica", async () => {
    ...
    expect((erro as AspectUnavailableError).available).toEqual(["corporate_events", "indicators", "facts"]);
  });
});
...
describe("ligação derivada do contrato", () => {
  ...
  test("caminho vem do path do contrato: o que está no path é posicional, o resto é query", async () => {
    const { db, chamadas } = clienteFalso({
      listCompanyDocuments: (async (...a: unknown[]) => (chamadas.push({ op: "listCompanyDocuments", args: a }), {})) as never,
      getObject: (async () => ({ id: "pub_c", kind: "company", subkind: null, name: "CIA", keys: [], links: [], declared_facts: [], aspects: [{ name: "documents", description: "", grain: "object", shape: "event", operation: "listCompanyDocuments", parameter: "company", value: "9512", defaults: { company: "9512" } }] })) as never,
    });
    const i = await new Objects(db).get("pub_c");
    await i.aspect("documents", { category: "FRE" });
    // `company` está no PATH de `/companies/{company}/documents`: vira posicional, e o filtro do
    // chamador desce para a query.
...
describe("recorte pela contraparte em `links()`", () => {
  // O gate P83: o mesmo verbo aponta para tipos diferentes, e sem o recorte no SERVIDOR a
  // única saída é paginar tudo e descartar no cliente. `related()` continua filtrando na mão
  // de propósito — ele reaproveita UMA leitura entre `securities()`, `instruments()` e
  // `issued()`, e empurrar o tipo para a query transformaria essa leitura em três.
  test("`other_kind` e `other_subkind` viajam na query, não são filtrados depois", async () => {
    const { objects, chamadas } = montar();
    const cedente = await objects.resolveOne("33000167000101", { kind: "company" });
    await cedente.links("assigned_to", { other_kind: "fund", other_subkind: "fidc", total: true });
    expect(chamadas.at(-1)?.args[1]).toMatchObject({
      rel: "assigned_to",
      direction: "out",
      other_kind: "fund",
      other_subkind: "fidc",
  ...
  test("sem o recorte, os dois campos saem indefinidos — a query não ganha parâmetro vazio", async () => {
    const { objects, chamadas } = montar();
    const cedente = await objects.resolveOne("33000167000101", { kind: "company" });
    await cedente.links("assigned_to");
    const p = chamadas.at(-1)?.args[1] as { other_kind?: string; other_subkind?: string };
    expect([p.other_kind, p.other_subkind]).toEqual([undefined, undefined]);
  });
});
