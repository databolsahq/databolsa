...
/**
 * As funções que o TIPO publica (`market.corporate_events.list`) — ESTRITO: função que o manifesto não
...
/**
 * O ÚLTIMO argumento do método flat (as opções), por aridade: os métodos do contrato podem ter
 * até dois parâmetros de caminho antes das opções (`metodo(a, b, opts?)`).
 ...
 * opcionais — o caminho vem de `defaults`. É o MESMO tipo que `db.listCorporateEvents(ticker,
 * opts)` aceita, então `from: string` e `perAgency: boolean` valem aqui como lá.
 */
...
export interface HistoryOpts extends RangeOpts {
  ...
  series?: string;
  /**
   * Transformação calculada sobre a série, guardada pela classe `axes.aggregation`: fluxos somam
   * (`sum_12m`, `ytd`), variações compõem (`compound_12m`, `compound_ytd`), níveis e taxas tiram
   * média (`mean_3m`, `mean_12m`) ou diferença (`diff`, em p.p.); `pct_change` é a variação %
   * contra o ponto anterior. Classe que não sustenta a conta chega como `error` na série.
   */
...
export interface LinksOpts {
  direction?: "out" | "in";
  /**
   * O recorte pela CONTRAPARTE, feito no servidor antes da paginação. É o que separa os dois
   * universos que um mesmo verbo mistura: `assigned_to` de um cedente aponta ora para série de
   * CRI, ora para FIDC, e sem o recorte achar o FIDC custava cinco páginas de 331 arestas.
   */
  other_kind?: Loose<ObjectKind>;
...
export class ObjectHandle<K extends string = string> {
  ...
  links(rel: Loose<Rel>, opts: LinksOpts = {}): Promise<ObjectLinksResponse> {
    return this.ctx.db.listObjectLinks(this.id, {
      ...
      other_kind: opts.other_kind,
      other_subkind: opts.other_subkind,
      at: this.asOf,
      limit: opts.limit,
      total: opts.total,
      cursor: opts.cursor,
  ...
  /**
   * Executa uma FUNÇÃO do registry pelo id, tipada pelo manifesto:
   * `petr4.fn("market.corporate_events.list", { from })` sabe que os parâmetros são os de
   * `listCorporateEvents` e devolve `CorporateEventsResponse`. É a porta
...
/** id da função → nome do capítulo, pelo manifesto (`market.corporate_events.list` → `corporate_events`). */
 ...
 * `event` virava `to` em operação que não publica `to`, e o vigente passava por resposta
 * daquela data.
 */
/**
