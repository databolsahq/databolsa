...
describe("1. `fn()` — a função do registry, tipada pelo manifesto", () => {
  test("parâmetros e resposta vêm do método flat que serve a função; o caminho vem de `defaults`", async () => {
    ...
    // `adjusted: boolean` compila porque o tipo é o de `db.listQuotes(ticker, opts)`, não a query crua
    await petr4.fn("market.quotes.history", { from: "2025-01-01", adjusted: false });
    expect(chamadas.at(-1)).toEqual({ op: "listQuotes", args: ["PETR4", { from: "2025-01-01", adjusted: false }] });
    const cia = await objects.resolveOne("33000167000101", { kind: "company" });
    await cia.fn("credit.ratings.list", { perAgency: true, limit: 20 });
    expect(chamadas.at(-1)).toEqual({ op: "listCreditRatings", args: [{ issuerCnpj: "33000167000101", perAgency: true, limit: 20 }] });
    // prova de tipo: a função da companhia é servida por `listCreditRatings`
    const op: FunctionOperations["credit.ratings.list"] = "listCreditRatings";
    expect(op).toBe("listCreditRatings");
  });

  test("função que o tipo não publica é AspectUnavailableError listando as funções que existem", async () => {
    const { objects } = montar();
    const petr4 = await objects.resolveOne("PETR4", { kind: "equity_security" });
    // `credit.ratings.list` não é função de papel: o tipo recusa, e em runtime o erro diz o que há
    const erro = await petr4.fn("credit.ratings.list" as never).catch((e: unknown) => e);
    expect(erro).toBeInstanceOf(AspectUnavailableError);
    expect((erro as AspectUnavailableError).message).toContain("market.quotes.history");
  });
  ...
  test("mapa de servidor antigo, sem `function`, ainda resolve pelo nome do capítulo", async () => {
    ...
    await (petr4 as HandleOf<"equity_security">).fn("market.quotes.history");
    expect(chamadas.at(-1)?.op).toBe("listQuotes");
  });
  ...
  test("os atalhos por tipo são só nomes de domínio sobre `fn()` — mesma chamada, mesmos parâmetros", async () => {
    ...
    await petr4.fn("market.quotes.history", { from: "2025-01-01" });
    const [a, b] = chamadas.filter((c) => c.op === "listQuotes");
    expect(a).toEqual(b!);
  });
});
...
describe("7. `perAgency` viaja no fio", () => {
  test("HttpClient manda `perAgency=true` na query — o adapter mandava e o cliente descartava", async () => {
    const original = globalThis.fetch;
    const urls: string[] = [];
    globalThis.fetch = (async (input: string | URL | Request) => {
      urls.push(String(input));
      return new Response(JSON.stringify({ data: [], meta: { next_cursor: null, count: 0 } }), { status: 200, headers: { "content-type": "application/json" } });
    }) as typeof fetch;
    try {
      const db = new HttpClient("https://api.example");
      await db.listCreditRatings({ issuerCnpj: "33592510000154", perAgency: true });
      await db.listCreditRatings({ issuerCnpj: "33592510000154" });
    } finally {
      globalThis.fetch = original;
    }
    expect(urls[0]).toContain("perAgency=true");
    expect(urls[1]).not.toContain("perAgency");
  });
});
...
describe("revisão da Etapa 2 — o handle é o sujeito e o tempo", () => {
  test("[P1] o parâmetro de sujeito não troca o objeto: outro issuerCnpj é SubjectOverrideError", async () => {
    ...
    await expect(cia.fnAny("credit.ratings.list", { issuerCnpj: "33592510000154" })).rejects.toBeInstanceOf(SubjectOverrideError);
    const petr4 = await objects.resolveOne("PETR4", { kind: "equity_security" });
    // @ts-expect-error `ticker` é o sujeito e não existe em FunctionParams<"market.quotes.history">
    await expect(petr4.fn("market.quotes.history", { ticker: "VALE3" })).rejects.toBeInstanceOf(SubjectOverrideError);
    expect(chamadas.filter((c) => c.op === "listCreditRatings" || c.op === "listQuotes")).toHaveLength(0);
  });
  ...
  test("[P1] o corte do handle vence: `to` divergente é TemporalConflictError, igual é aceito", async () => {
    ...
    await expect(em2024.market.quotes.history({ to: "2026-01-01" })).rejects.toBeInstanceOf(TemporalConflictError);
    await em2024.market.quotes.history({ to: "2024-12-31", from: "2024-01-01" });
    expect(chamadas.at(-1)).toEqual({ op: "listQuotes", args: ["PETR4", { to: "2024-12-31", from: "2024-01-01" }] });
  ...
  test("[P2] `fn()` é estrito em compilação; `fnAny()` é a porta explícita para função nova", async () => {
    const { objects, chamadas } = montar();
    const petr4 = await objects.resolveOne("PETR4", { kind: "equity_security" });
    // @ts-expect-error `credit.ratings.list` não é função de equity_security
    await petr4.fn("credit.ratings.list").catch(() => undefined);
    // `market.quotes.history` é função do papel e continua; `fnAny` é a porta para o id
    // que o tipo ainda não conhece.
    await petr4.fnAny("market.quotes.history", { from: "2025-01-01" });
    expect(chamadas.at(-1)?.op).toBe("listQuotes");
  });
...
describe("o grão e o tipo governam a assinatura", () => {
  test("`company.market.quotes.history` exige `series` em compilação; `fn` genérico ainda recusa em runtime", async () => {
    const { objects, chamadas } = montar();
    const cia = await objects.resolveOne("33000167000101", { kind: "company" });
    // @ts-expect-error o grão é de papel: sem `series` não compila — e, forçado, é AmbiguousPaperError em runtime
    await expect(cia.market.quotes.history()).rejects.toBeInstanceOf(AmbiguousPaperError);
    await cia.market.quotes.history({ series: "PETR4", from: "2025-01-01" });
