 ...
 *
 *   equity_security → market.quotes.history
 *   company → issued → instrument → credit.ratings.list
 *
 * Aspect é acessor direto (`market.quotes`, `credit.ratings`), relação tem nome de domínio
 * (`issuer()`, `instruments()`), e o grão aparece na assinatura: `company.market.quotes`
 * exige escolher o papel, `equity_security.market.quotes` não.
 */

/** As opções de cada atalho SÃO as da função no registry — nada declarado à mão. */
export type QuotesOpts = FunctionParams<"market.quotes.history">;
...
export class EquitySecurityHandle extends ObjectHandle<"equity_security"> {
  ...
  readonly market = {
    quotes: {
      /** Cotações diárias, ajustadas por proventos por default. */
      history: (opts: QuotesOpts = {}) => this.fn("market.quotes.history", opts),
    },
    indicators: {
...
export class CompanyHandle extends ObjectHandle<"company"> {
  readonly market = {
    quotes: {
      /**
       * Cotações DE UM PAPEL da companhia: o grão é de papel, então `series` (ticker ou classe) é
       * obrigatório NA ASSINATURA — "o preço da Petrobras" não existe, existe o de PETR3 e o de
       * PETR4. Companhia de um papel só também escolhe; é o que a torna explícita.
       */
      history: (opts: QuotesOpts & { series: string }) => {
        // Chamada sem `series` (JS sem tipos, ou `as never`) cai no runtime: é `AmbiguousPaperError`, não TypeError.
        const { series, ...resto } = opts ?? ({} as QuotesOpts & { series: string });
        return this.fn("market.quotes.history", resto, { series });
      },
    },
  };
