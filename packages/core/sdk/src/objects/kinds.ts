 ...
 *   company → issued → instrument (a nota da agência virou a aresta `rates` em 04/09/2026)
 *
 * Aspect é acessor direto (`market.corporate_events`), relação tem nome de
...
export class InstrumentHandle extends ObjectHandle<"instrument"> {
  // Sem atalho de função: a nota da agência é a aresta `rates` (`links("rates", "in")`) desde
  // 04/09/2026, e o secundário é série de fatos do próprio instrumento.
