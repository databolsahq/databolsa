...
// Jornada 1 — papel → eventos societários, fatos, emissor
const petr4 = await db.objects.resolveOne("PETR4", { kind: "equity_security" });
console.log(`${petr4.name} (${petr4.id})`);

const eventos = await petr4.market.corporate_events.list({ from: "2025-01-01", limit: 3 });
console.log("eventos societários:", eventos.data.length, "linhas; primeira:", eventos.data[0]);
...
// O grão na assinatura: "o desdobramento da Petrobras" não existe — o SDK obriga a escolher o papel.
try {
  await issuer.market.corporate_events.list();
} catch (e) {
  console.log("esperado:", (e as Error).message);
}
const petr3 = await issuer.paper("PETR3");
console.log("papel escolhido:", petr3.name, (await petr3.market.corporate_events.list({ limit: 1 })).data[0]?.ex_date);
