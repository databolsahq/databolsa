...
// Layout de FONTE no repo: packages/core/mcp/src/openapi.ts.
const SOURCE = "file:///home/u/databolsa/packages/core/mcp/src/openapi.ts";
...
test("publicado: candidato aponta p/ o openapi.yaml ao lado do bundle (dist/)", () => {
  ...
  // Com packages/<grupo>/<pacote> a fonte fica 4 níveis abaixo da raiz; no publicado o
  // 2º candidato cai fora do node_modules (inexistente, inofensivo — o 1º é o que vale).
  expect(cands).toContain("/home/u/proj/api/openapi.yaml");
});
...
test("extrai schema da resposta 2xx e registry de components para outputSchema", () => {
  const op = extractOperations(FAKE_SPEC).find((item) => item.operationId === "getThing")!;
  expect(op.responseSchema?.$ref).toBe("#/components/schemas/Thing");
  expect(op.schemas?.Thing?.properties?.id?.type).toBe("string");
});
