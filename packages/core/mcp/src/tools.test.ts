...
test("annotations classificam leitura, escrita aditiva e escrita destrutiva", () => {
  ...
  expect(annotate("post", "importPortfolioFile")).toEqual({
    readOnlyHint: false,
    openWorldHint: true,
    destructiveHint: false,
  });
  // POST que sobrescreve estado existente entra em DESTRUCTIVE_POSTS à mão — o método
  // sozinho não distingue "cria" de "reescreve".
  expect(annotate("post", "reconcilePortfolioAsset")).toEqual({
    readOnlyHint: false,
    openWorldHint: true,
    destructiveHint: true,
  });
