...
describe("executeObjectFunction no MCP", () => {
  test("só entra quando a sessão enxerga o grafo — perfil sem getObject fica sem a tool", () => {
    expect(sessaoComGrafo([op("getObject"), op("listCorporateEvents")])).toBe(true);
    expect(sessaoComGrafo([op("listCorporateEvents"), op("getStockIndicators")])).toBe(false);
  });
  ...
  test("a tool é leitura pura e valida o input ANTES de qualquer chamada", async () => {
    const tool = buildObjectFunctionTool({
      ...
      apiKey: "k",
      operations: [op("listCorporateEvents", [{ name: "ticker", in: "path", type: "string", required: true }, { name: "from", in: "query", type: "string", required: false }])],
    });
    ...
    // `bogus` não é input de market.corporate_events.list: a recusa precisa vir da validação, sem
    // rede — o teste não tem servidor nenhum atrás de api.example.test.
    const r = await tool.handler({ subject: { resolve: "PETR4" }, function: "market.corporate_events.list", input: { bogus: 1 } });
    expect(r.isError).toBe(true);
    expect((r.content[0] as { text: string }).text).toContain("não é input de market.corporate_events.list");
  });
  ...
  test("o catálogo de funções vai na descrição — o cliente lê o registry antes de escolher", () => {
    ...
    expect(tool.config.description).toContain("market.corporate_events.list");
...
describe("o catálogo na descrição não vira enumeração longa", () => {
  const inputs = inputsDoContrato([op("listCorporateEvents", [{ name: "from", in: "query", type: "string", required: false }])]);

  test("abaixo do teto: a linha por função, com tipos e inputs", () => {
    const texto = catalogoDeFuncoes(inputs);
    expect(texto).toContain("market.corporate_events.list → ");
