...
export interface RunCliConfig {
  ...
  /**
   * MÓDULOS (CLI agregadora): `databolsa wallet listPortfolios` roda a operação no contrato
   * da extensão, não no core. O executável não incorpora as extensões: só sabe onde está o
   * contrato de cada uma (vivo + fallback bundlado) e delega ao mesmo motor.
   */
  modules?: Record<string, CliModule>;
}

export interface CliModule {
  /** Path do spec vivo da extensão (ex.: /openapi-wallet.json). */
  specPath: string;
  staticSpecCandidates?: string[];
  /** Uma linha para o `--help` de topo. */
  blurb: string;
}

/** Puro: `wallet listPortfolios` → módulo `wallet` + argv restante; sem módulo, argv intacto. */
export function splitModule(argv: string[], modules: Record<string, CliModule> | undefined): { module?: string; argv: string[] } {
  const first = argv[0];
  if (modules && first && !first.startsWith("-") && first in modules) return { module: first, argv: argv.slice(1) };
  return { argv };
}
...
async function main(config: RunCliConfig): Promise<void> {
  ...
  const { module, argv } = splitModule(process.argv.slice(2), config.modules);
  const mod = module ? config.modules![module]! : undefined;
  ...
  if (!g.command && !g.list) {
    process.stdout.write(topUsage(config.modules ? Object.fromEntries(Object.entries(config.modules).map(([k, m]) => [k, m.blurb])) : undefined));
    return;
  }

  // O workspace NÃO é escolhido aqui. Toda chave `db_live_` nasce presa a um workspace, e a
  // API age nele; um cliente que pudesse apontar a mesma chave para outra organização era o
  // caminho para uma leitura ou escrita pousar no lugar errado. Para agir noutro workspace,
  // emita uma chave lá.
  const api = new ApiClient({
    baseUrl: g.apiUrl ?? process.env[b.envUrl],
    apiKey: process.env[b.envKey],
    specPath: mod?.specPath ?? config.specPath,
  });

  const operations = await loadOperations(api, { staticCandidates: mod ? mod.staticSpecCandidates : config.staticSpecCandidates });
