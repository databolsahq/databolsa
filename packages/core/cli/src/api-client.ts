...
export interface ApiClientOptions {
  ...
  apiKey?: string | null;
  /** Path do spec vivo na origem (default `/openapi.json`). */
  specPath?: string;
  /** Headers extras em toda requisição (ex.: `x-databolsa-workspace`). */
...
export class ApiClient {
  ...
  /** Path do spec vivo na origem (default `/openapi.json`; advisor: `/openapi-advisor.json`). */
  ...
  constructor(opts: ApiClientOptions = {}) {
    ...
    this.specPath = opts.specPath ?? "/openapi.json";
  }
  ...
  /** Requisição genérica (GET/POST/PUT/PATCH/DELETE) — operações de escrita mandam body JSON. */
  async request(
    ...
    return { ok: false, status: res.status, body: parsed, detail: problemDetail(parsed) ?? `HTTP ${res.status}` };
  }

  /** Busca o spec OpenAPI vivo (JSON) servido pela API. */
  async fetchOpenApi(): Promise<unknown> {
    const res = await fetch(`${this.origin}${this.specPath}`, { headers: { accept: "application/json" } });
    if (!res.ok) throw new Error(`GET ${this.origin}${this.specPath} → HTTP ${res.status}`);
    return res.json();
  }
}
