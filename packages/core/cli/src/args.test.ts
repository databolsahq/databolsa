...
test("faixa numérica do contrato é validada antes da API", () => {
  expect(messageOf(expectations, ["--indicator", "ipca", "--limit", "0"])).toContain(
    "maior ou igual a 1",
  );
  expect(messageOf(expectations, ["--indicator", "ipca", "--limit", "101"])).toContain(
    "menor ou igual a 100",
  );
  expect(bindArgs(expectations, ["--indicator", "ipca", "--limit", "100"]).query.limit).toBe(100);
});
