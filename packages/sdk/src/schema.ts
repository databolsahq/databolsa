...
export interface components {
    schemas: {
        ...
        /** @description Posição computada da carteira. Contrato LEGADO em camelCase (estável até a próxima major; novos contratos do /v1 usam snake_case). Equivalências com o ledger (listPortfolioTransactions): assetType ↔ asset.asset_type; transactions[].date ↔ trade_date. O objeto carrega campos computados adicionais (avg, investedCost, unrealizedPL, realizedPL, incomeReceived, dayChangeBrl, closed, isWatchlist…) além dos listados. */
        PortfolioHolding: {
            symbol: string;
            /** @description stock | fii | etf | bdr | fund | tesouro | rf | crypto | us — igual a asset.asset_type do ledger. */
            assetType: string;
            name?: string;
            qty: number;
            ...
            /** @description Transações da posição (visão computada). `date` equivale a `trade_date` do ledger. */
        ...
        /** @description Carteira computada: posições derivadas do ledger + métricas agregadas. Contrato LEGADO em camelCase (estável até a próxima major; novos contratos do /v1 usam snake_case). */
        PortfolioDetail: {
            id?: string;
            name?: string;
            patrimonio?: number;
        ...
        /** @description Transação do ledger (snake_case — convenção canônica do /v1). `trade_date` equivale ao `date` das transações embutidas em PortfolioHolding (contrato legado camelCase). */
