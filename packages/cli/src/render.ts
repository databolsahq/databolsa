...
/**
 * Demais chaves de um wrapper paginado viram um rodapé enxuto. `meta` é achatado:
 * ele carrega a paginação E o contexto da consulta (ticker, sessão, indicador), que
 * como um blob JSON numa linha só ninguém lê.
 */
function renderFooter(obj: Record<string, unknown>, omit: string): string {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (k === "meta" && v !== null && typeof v === "object" && !Array.isArray(v)) {
      for (const [mk, mv] of Object.entries(v as Record<string, unknown>)) push(mk, mv);
      continue;
    }
    push(k, v);
  }
