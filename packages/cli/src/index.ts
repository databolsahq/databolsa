...
import { buildCommands, commandHelp, findCommand, listText, topUsage, usageLine } from "./commands";
...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    `Erro da API (HTTP ${res.status})${res.detail ? `: ${res.detail}` : ""}.\n${apiErrorHint(res.status)}`,
  );
  process.exitCode = 1;
}

/** O que fazer a seguir nos erros em que a ação é sempre a mesma. */
...
main().catch((err: unknown) => {
  if (err instanceof CliError) {
    // Só a linha de uso, não o help inteiro: a mensagem já nomeia o valor rejeitado
    // e o que é aceito, e despejar a ajuda completa embaixo dela enterra o erro.
    process.stderr.write(`${err.message}\n`);
    if (err.spec) {
      process.stderr.write(
        `Uso: ${usageLine(err.spec)}\nAjuda: databolsa ${err.spec.operationId} --help\n`,
      );
    }
    process.exitCode = 2;
    return;
