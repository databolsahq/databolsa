 ...
 */
function normalizeType(schema: { type?: string | string[]; $ref?: string; items?: unknown; properties?: unknown } | undefined): ParamSpec["type"] {
  const t = schema?.type;
  ...
  if (v === "object" || (!v && (schema?.properties || schema?.$ref))) return "object";
  return "string";
}
