...
// Gate de release: roda TUDO contra a API local (boot do packages/api), sem tocar a API
// pública. Pega pacote/instalação/bin/import/MCP/auth quebrados sem depender de rede externa.
...
try {
  if (mode === "flow" || mode === "flow-min") {
    ...
    if (mode === "flow-min") {
      ...
      console.log("SDK_FLOW_OK status=" + h.status + " indices=" + idx.length);
    }
  } else if (mode === "auth") {
...
/**
 * Sobe uma instância LOCAL do packages/api (sem banco) e roda `fn(local, key)` contra
 * ela, garantindo o teardown. A API é o ponto de apoio tanto da matriz de auth quanto
 * do flow smoke no modo --no-live — nenhuma dependência de rede externa.
 */
...
// ---- main -------------------------------------------------------------------
async function main() {
  const where = NO_LIVE ? "local-only (gate)" : `api=${API_URL}`;
