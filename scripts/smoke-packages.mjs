...
try {
  if (mode === "flow" || mode === "flow-min") {
    ...
    if (mode === "flow-min") {
      ...
      const q = await c.listQuotes("PETR4", { limit: 5 });
      if (!JSON.stringify(q).includes("PETR4")) throw new Error("listQuotes PETR4 inesperado");
...
try {
  ...
  for (const m of ["listQuotes", "getHealth", "listObjects"]) {
...
async function offlineSmoke(consumer) {
  ...
  await check("MCP responde tools/list com tools", async () => {
    ...
    assert(tools.some((t) => t.name === "listQuotes"), "tool listQuotes ausente");
 ...
 * (listQuotes PETR4, índices); com `realData=false` (API local sem banco) valida só o
...
async function flowSmoke(consumer, target, realData) {
  ...
  if (realData) {
    await check("CLI listQuotes PETR4 retorna o ticker", () => {
      const r = run(binDatabolsa, ["listQuotes", "PETR4", "--limit", "5", "--json"], { env });
