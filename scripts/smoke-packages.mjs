#!/usr/bin/env node
/**
 * smoke-packages.mjs — teste de fumaça dos clientes publicados (core sdk·cli·mcp e os
 * das extensões: advisor, wallet e mesa de crédito).
 *
 ...
 *   --version=<x.y.z>  versão a instalar no modo registry (default: a do packages/core/sdk)
...
const PKGS = [
  "sdk", "cli", "mcp",
  "advisor-sdk", "advisor-mcp", "advisor-cli",
  "wallet-sdk", "wallet-mcp", "wallet-cli",
  "credit-sdk", "credit-mcp", "credit-cli",
];
// Onde cada cliente mora: os do contrato core em packages/core/<pacote>; os das extensões
// em extensions/<extensão>/<pacote> (taxonomia de 02/09/2026 — uma pasta por extensão).
const dirDe = (p) =>
  p.startsWith("advisor-")
    ? join(REPO, "extensions", "advisor", p)
    : p.startsWith("wallet-")
      ? join(REPO, "extensions", "wallet", p)
      : p.startsWith("credit-")
        ? join(REPO, "extensions", "credit", p)
        : join(REPO, "packages", "core", p);

// No espelho público só existem os clientes publicados (core, Advisor, Wallet): pacote cuja
// pasta não está na árvore (a mesa de crédito ainda não é pública) sai da rodada, nomeado.
const PKGS_PRESENTES = PKGS.filter((p) => existsSync(join(dirDe(p), "package.json")));
for (const p of PKGS) if (!PKGS_PRESENTES.includes(p)) console.log(`  · ${p}: fora desta árvore, pulado`);
...
const VERSION = String(flag("version", readJson(join(REPO, "packages/core/sdk/package.json")).version));
...
function buildIsolatedEnv(workdir) {
  ...
  let installArgs;
  if (SOURCE === "pack") {
    log(`==> npm pack (${PKGS_PRESENTES.join(", ")}) — prepack builda o dist`);
    const tarballs = [];
    for (const p of PKGS_PRESENTES) {
      ...
      const ver = readJson(join(dirDe(p), "package.json")).version;
      run("npm", ["pack", "--pack-destination", workdir], { cwd: dirDe(p) });
    ...
    installArgs = PKGS_PRESENTES.map((p) => `@databolsa/${p}@${VERSION}`);
  }
...
try {
  if (mode === "flow" || mode === "flow-min") {
    const c = new HttpClient(url, { apiKey: process.env.DATABOLSA_API_KEY || null });
    ...
    if (mode === "flow-min") {
      ...
      const r = await c.resolveObject({ q: "PETR4", kind: "equity_security" });
      const petr4 = r?.data?.[0]?.id;
      if (!petr4) throw new Error("resolveObject PETR4 sem candidato");
      const q = await c.getObjectHistory(petr4, { facts: "close", limit: 5 });
      if (!Array.isArray(q?.data) || q.data.length === 0) throw new Error("getObjectHistory(close) vazio");
      const idx = await c.listObjects({ kind: "index", limit: 100 });
      if (!Array.isArray(idx?.data) || idx.data.length === 0) throw new Error("listObjects(kind=index) vazio");
      ...
      const cr = await c.listObjects({ kind: "crypto_asset", limit: 5 });
      if (!Array.isArray(cr?.data)) throw new Error("listObjects(kind=crypto_asset) sem data[]");
      const us = await c.listObjects({ kind: "company", where: "market=us", limit: 5 });
      if (!Array.isArray(us?.data)) throw new Error("listObjects(kind=company, market=us) sem data[]");
      console.log(
        "SDK_FLOW_OK status=" + h.status + " indices=" + idx.data.length +
    ...
    try {
      await c.listObjects({ kind: "index", limit: 1 });
...
try {
  ...
  for (const m of ["getObjectHistory", "getHealth", "listObjects"]) {
    if (typeof c[m] !== "function") throw new Error("HttpClient sem " + m);
  }
  if (typeof extractOperations !== "function" || typeof describeOperation !== "function") {
    throw new Error("subpath @databolsa/sdk/openapi incompleto");
  }
...
async function offlineSmoke(consumer) {
  ...
  await check("MCP responde tools/list com tools", async () => {
    ...
    assert(tools.some((t) => t.name === "getObjectHistory"), "tool getObjectHistory ausente");
    return `${tools.length} tools`;
  });

  await check("Advisor SDK importa e instancia o client", () => {
    const advRunner = join(consumer, "advisor-sdk-runner.mjs");
    writeFileSync(
      advRunner,
      `import { DataBolsaAdvisor } from "@databolsa/advisor-sdk";
const c = new DataBolsaAdvisor({ baseUrl: "http://127.0.0.1:1" });
for (const m of ["getMe", "listClients", "getPortfolio"]) {
  if (typeof c[m] !== "function") { console.error("ADVISOR_SDK_FAIL: sem " + m); process.exit(1); }
}
console.log("ADVISOR_SDK_OK");
`,
    );
    const r = run("node", [advRunner], { cwd: consumer });
    const line = r.stdout.trim().split(/\r?\n/).pop();
    assert(line === "ADVISOR_SDK_OK", `saída inesperada: ${line}`);
    return "ok";
  });

  await check("Advisor CLI --version casa com a do pacote", () => {
    const binAdvisorCli = join(consumer, "node_modules", ".bin", "databolsa-advisor");
    const r = run(binAdvisorCli, ["--version"]);
    const v = r.stdout.trim();
    assert(/^\d+\.\d+\.\d+/.test(v), `versão inesperada: ${v}`);
    return v;
  });

  await check("Advisor MCP responde tools/list (fallback estático)", async () => {
    const binAdvisorMcp = join(consumer, "node_modules", ".bin", "databolsa-advisor-mcp");
    const advEnv = { ...process.env, DATABOLSA_ADVISOR_API_URL: "http://127.0.0.1:1", DATABOLSA_ADVISOR_API_KEY: "" };
    const msg = await mcpHandshake(binAdvisorMcp, advEnv);
    const tools = msg?.result?.tools;
    assert(Array.isArray(tools) && tools.length > 0, "tools/list vazio");
    assert(tools.some((t) => t.name === "advisorGetMe"), "tool advisorGetMe ausente");
    assert(tools.every((t) => /^advisor[A-Z]/.test(t.name)), "tool sem prefixo advisor* no contrato Advisor");
    return `${tools.length} tools`;
  });

  if (PKGS_PRESENTES.includes("credit-sdk")) await check("Credit SDK importa e instancia o client", () => {
    const runner = join(consumer, "credit-sdk-runner.mjs");
    writeFileSync(
      runner,
      `import { DataBolsaCredit } from "@databolsa/credit-sdk";
const c = new DataBolsaCredit({ baseUrl: "http://127.0.0.1:1" });
for (const m of ["getMe", "listWatchlists", "listOrgRegulationTerms"]) {
  if (typeof c[m] !== "function") { console.error("CREDIT_SDK_FAIL: sem " + m); process.exit(1); }
}
console.log("CREDIT_SDK_OK");
`,
    );
    const r = run("node", [runner], { cwd: consumer });
    const line = r.stdout.trim().split(/\r?\n/).pop();
    assert(line === "CREDIT_SDK_OK", `saída inesperada: ${line}`);
    return "ok";
  });

  if (PKGS_PRESENTES.includes("credit-cli")) await check("Credit CLI --version casa com a do pacote", () => {
    const bin = join(consumer, "node_modules", ".bin", "databolsa-credit");
  ...
  if (PKGS_PRESENTES.includes("credit-mcp")) await check("Credit MCP responde tools/list (fallback estático)", async () => {
    const bin = join(consumer, "node_modules", ".bin", "databolsa-credit-mcp");
    const creditEnv = { ...process.env, DATABOLSA_CREDIT_API_URL: "http://127.0.0.1:1", DATABOLSA_CREDIT_API_KEY: "" };
    const msg = await mcpHandshake(bin, creditEnv);
    const tools = msg?.result?.tools;
    assert(Array.isArray(tools) && tools.length > 0, "tools/list vazio");
    assert(tools.some((t) => t.name === "deskGetMe"), "tool deskGetMe ausente");
    assert(tools.every((t) => /^desk[A-Z]/.test(t.name)), "tool sem prefixo desk* no contrato do Credit");
    return `${tools.length} tools`;
  });

  await check("Wallet SDK importa e instancia o client", () => {
    const runner = join(consumer, "wallet-sdk-runner.mjs");
    writeFileSync(
      runner,
      `import { DataBolsaWallet } from "@databolsa/wallet-sdk";
const c = new DataBolsaWallet({ baseUrl: "http://127.0.0.1:1" });
for (const m of ["listPortfolios", "getPortfolioDetail", "addPortfolioTransaction"]) {
  if (typeof c[m] !== "function") { console.error("WALLET_SDK_FAIL: sem " + m); process.exit(1); }
}
console.log("WALLET_SDK_OK");
`,
    );
    const r = run("node", [runner], { cwd: consumer });
    const line = r.stdout.trim().split(/\r?\n/).pop();
    assert(line === "WALLET_SDK_OK", `saída inesperada: ${line}`);
    return "ok";
  });

  await check("Wallet CLI --version casa com a do pacote", () => {
    const r = run(join(consumer, "node_modules", ".bin", "databolsa-wallet"), ["--version"]);
    const v = r.stdout.trim();
    assert(/^\d+\.\d+\.\d+/.test(v), `versão inesperada: ${v}`);
    return v;
  });

  await check("Wallet MCP responde tools/list (fallback estático)", async () => {
    const bin = join(consumer, "node_modules", ".bin", "databolsa-wallet-mcp");
    const msg = await mcpHandshake(bin, { ...process.env, DATABOLSA_API_URL: "http://127.0.0.1:1", DATABOLSA_API_KEY: "" });
    const tools = msg?.result?.tools;
    assert(Array.isArray(tools) && tools.length > 0, "tools/list vazio");
    assert(tools.some((t) => t.name === "listPortfolios"), "tool listPortfolios ausente");
    assert(tools.every((t) => /Portfolio|Suitability/.test(t.name)), "tool fora do contrato da Wallet");
    return `${tools.length} tools`;
  });
}
 ...
 * (a série de fechamento de PETR4 pelo objeto, índices); com `realData=false` (API local sem banco) valida só o
...
async function flowSmoke(consumer, target, realData) {
  ...
  if (realData) {
    await check("CLI resolveObject PETR4 retorna o objeto do papel", () => {
      const r = run(binDatabolsa, ["resolveObject", "--q", "PETR4", "--kind", "equity_security", "--json"], { env });
      assert(r.stdout.includes("PETR4"), "PETR4 ausente na saída");
      return "ok";
    });

    await check("CLI listObjects --kind index --json devolve um array", () => {
      const r = run(binDatabolsa, ["listObjects", "--kind", "index", "--json"], { env });
      const data = JSON.parse(r.stdout);
      const rows = Array.isArray(data) ? data : (data?.data ?? []);
      assert(Array.isArray(rows) && rows.length > 0, "listObjects(kind=index) não retornou array não-vazio");
      return `${rows.length} índices`;
    });
  } else {
...
async function withLocalApi(fn) {
  ...
  const apiEntry = join(REPO, "packages", "core", "api", "src", "index.ts");
  const env = {
    ...process.env,
    PORT: String(port),
...
async function authMatrix(consumer, local, KEY) {
  ...
  await check("/v1/health é público (200)", async () => {
    ...
    await check("chave certa → passa pela auth (≠ 401)", async () => {
      const r = await fetch(`${local}/v1/objects?kind=index`, { headers: { authorization: `Bearer ${KEY}` } });
    ...
    await check("CLI com chave errada surfaça 401", () => {
      const r = spawnSync(binDatabolsa, ["listObjects", "--kind", "index"], {
        encoding: "utf8",
